...
savesRoutes.get('/:gameId', async (req, res) => {
  try {
    ...
    console.error('[saves] GET /:gameId error:', err);
...
savesRoutes.post('/:gameId/:slot', upload.single('state'), async (req, res) => {
  try {
    ...
    console.error('[saves] POST /:gameId/:slot error:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
savesRoutes.get('/:gameId/:slot', async (req, res) => {
  try {
    ...
    const buffer = await storageService.getSaveState(
      req.user!.id, req.params.gameId, slot
    );
    if (!buffer) {
    ...
    res.set('Content-Disposition', `attachment; filename=slot_${slot}.state`);
    res.send(buffer);
  } catch (err) {
    console.error('[saves] GET /:gameId/:slot error:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
savesRoutes.delete('/:gameId/:slot', async (req, res) => {
  try {
    ...
    console.error('[saves] DELETE /:gameId/:slot error:', err);
    res.status(500).json({ error: 'Internal server error' });
  }
});
